...
Computer2::Computer2(std::string name, char color): Computer(name, color) {};

step_t Computer2::make_step(const std::shared_ptr<Player> &enemy) const {
    
    // More smart strategy
    // 1. find any eating option
    // 2. optimal possible step (with no eating from enemy)
...
step_t Computer2::find_eating_option(const std::shared_ptr<Player> &enemy) const {
...
step_t Computer2::find_step_option(const std::shared_ptr<Player> &enemy) const {
    step_t bad_option = {{0,0}, {0,0}};
    step_t good_option = {{0,0}, {0,0}};
...
//                    return bad_option;
                    ...
                    bool good = true;
                    for (int j = 0; j < 4; j++) {
                        position_t cell = near_cells_2[j];
                        if (check_cell(cell, enemy) == ENEMY) {
                            good = false;
                        }
                    }

                    if (good) {
                        good_option = {pawn_pos, pos};
                        return good_option;
                    }
                }
            }
